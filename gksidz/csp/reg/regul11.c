...
vd  far Regul_11(vd){
    ...
    if (AOT050__RM!=AOT050__RV){
        ...
        if(AOT050__RV==AUTO){ Message(420); }
        else                { Message(421); AOT050__OM=AOT050__OV=WO_Skip[RN]; }
    }
    //-----------------------------------------------------------------------
    // установка задания
    if (AOT050__ZM <  IL         ){ AOT050__ZM = IL; }
    if (AOT050__ZM >  AIT050_AMV ){ AOT050__ZM = AIT050_AMV; }
    AOT050__ZV =  AOT050__ZM;
    // выход на клапан ручного управления
    if (AOT050__OM < 0  ){ AOT050__OM =0;   }
    if (AOT050__OM > 100){ AOT050__OM =100; }
    //-----------------------------------------------------------------------
    ...
    if (RegRUN==ON){
       ...
       if (AOT050__MV >= 0){ AOT050_OOV=100-AOT050__OV; }
       else                { AOT050_OOV=AOT050__OV; }
       //--------------------------------------------------------------------
       // дискретный регулятор
       AOT050__UV=RegulDigit(RN, AOT050_LPV, AOT050_HPV, AOT050__MV,
                                 AOT050__RV, AOT050__UM);
       //--------------------------------------------------------------------
       if(AOT050__MV< 0){
       ...
       if(AOT050__UV==RPLUS ){ DON053O__V=1; DON053C__V=0; }
       if(AOT050__UV==RMINUS){ DON053O__V=0; DON053C__V=1; }
       if(AOT050__UV==RSTOP ){ DON053O__V=0; DON053C__V=0; }
